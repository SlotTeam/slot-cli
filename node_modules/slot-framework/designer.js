...
function execJsonBinding(slotKey, uriBuildKey, fileContent, filename, binds/*jsonBindFile*/, callback) {
    ...
    for(var field in binds) {
         ...
         *      de esta forma todo será mas entendible
         */
                            ...
                            mkdirp(mcvFolder, function (err) {
                                if (err) console.error(err)
                                    ...
                                    for(var sl in slots[uriBuildKey]) {
                                        ...
                                        //var tempLayout = templates[path.join(path.join(process.cwd(), slotJson.webRootDir), slots[uriBuildKey][sl])];
                                        var tempLayout = slots[uriBuildKey][sl] == '/' ? "index.html" : slots[uriBuildKey][sl];
                                        tempLayout = templates[path.join(path.join(process.cwd(), slotJson.webRootDir), tempLayout)];
...
function start(port) {
    ...
    Util.startHeaderMsg("Designer", port, options);
}
/**
 * Export functions
 */
