 ...
 * @param onNoRoutable      Function executed when the resource is not a Slot component(Model, View, HTML Layout, ect..),
 *                          it's any other content like; HTML files, media content, images, text files, ect.
        ...
        try {
            ...
            if((filename.split(".")[filename.split(".").length-1]).toLowerCase() == "html") {
                ...
                var //modelC= "../../" + modelCFile.replace(/\\/g, "/"),    //<<== Model on client side
                    modelS= Util.appFullPath() + modelSFile.replace(/\\/g, "/"),      //<<== Model on server side
                ...
                console.log('Loading module [%s] on [%s]', restModule, process.cwd());
...
function start(port) {
    ...
    fs.readFile(clientControler ,'binary' /*'utf8'*/, function(err, buffer) {
        ...
        fs.readFile(path.join(process.cwd(), "slot.json".replace(/\//g, path.sep)) ,'binary' /*'utf8'*/, function(err, buffer) {
            ...
            options.caching = devMode;
            Util.startHeaderMsg("Development", port, options);
        });
    });
}
